...
REST APIのパス・ステータスコード・JSONキー・ヘッダ意味論はGo実装と揃えています。意図的な差異は2点です。outboxペイロードとAPI応答の`created_at`はISO 8601のサブ秒精度としてのみ規定し、末尾ゼロの有無など表現の細部は実装依存です（下流はこの値に依存しません）。エラー応答は「ステータスコードと`error`キーの存在」までを契約とし、日本語メッセージの文言は実装依存です。
